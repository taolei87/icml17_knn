...
def gated_wln(graph_inputs, batch_size, hidden_size, depth):
    input_atom, input_bond, atom_graph, bond_graph, num_nbs, node_mask = graph_inputs
    atom_features = tf.nn.relu(linearND(input_atom, hidden_size, "atom_embedding", init_bias=None))
    layers = []
    for i in xrange(depth):
        with tf.variable_scope("WL", reuse=(i>0)) as scope:
            fatom_nei = tf.gather_nd(atom_features, atom_graph)
            fbond_nei = tf.gather_nd(input_bond, bond_graph)
            h_nei_atom = linearND(fatom_nei, hidden_size, "fnei_atom", init_bias=None)
            h_nei_bond = linearND(fbond_nei, hidden_size, "fnei_bond", init_bias=None)
            g_nei_atom = linearND(fatom_nei, hidden_size, "gnei_atom", init_bias=None)
            g_nei_bond = linearND(fbond_nei, hidden_size, "gnei_bond", init_bias=None)
            g_self = tf.expand_dims(linearND(atom_features, hidden_size, "gself"), -2)
            g_nei = tf.nn.sigmoid(g_nei_atom + g_nei_bond + g_self) * 10
