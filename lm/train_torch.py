...
class Model(nn.Module):
    ...
    def init_weights(self):
        ...
        # pytorch lstm has two bias vectors, disable one of them
        #lstm_params = list(self.rnn.parameters())
        #assert len(lstm_params) == 4*self.depth
        #for i in range(self.depth):
        #    lstm_params[i*4+3].data.zero_()
        #    lstm_params[i*4+3].requires_grad = False
...
def eval_model(prefix, model, valid):
    ...
    ppl = np.exp(avg_loss)
    sys.stdout.write("\t[{}]  eval_loss={:.3f}  eval_ppl={:.1f}\n".format(
        prefix,
        avg_loss,
        ppl
    ))
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--lr_decay", type=float, default=0.9)
    argparser.add_argument("--lr_decay_epoch", type=int, default=20)
